...
export default function Slider({images} : SliderProps) {
  ...
  return (
    <div className="relative mt-20 h-[90vh] w-full overflow-hidden bg-gray-200">
      <AnimatePresence initial={false} custom={direction}>
        ...
        <motion.div
            ...
            className="absolute inset-0 bg-black/60 flex flex-col items-center justify-center text-white p-8 z-[19]"
            initial={{opacity: 0}}
            animate={{opacity: 1}}
            exit={{opacity: 0}}
      ...
      <div className="absolute inset-0 flex items-center justify-between p-4 z-[19]">
      ...
      <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-2 z-[80]">
            {images.map((_, index)=>(
                ...
                }} className={` h-3 rounded-full transition-all ${index === currentIndex ? "bg-white w-6" : "bg-white/50 w-3 hover:bg-white/80"}`} aria-label={`Aller à l'image ${index + 1}`}>
                </button>
            ))}
      </div>
    </div>
